...
class LoginForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError("this email id has not been registered")
        return email


    def clean(self):
        if 'email'  in self.cleaned_data :
            # raise forms.ValidationError("email id do not exists")
            self.user = authenticate(email=self.cleaned_data['email'], password=self.cleaned_data['password'])
            if self.user is None:
                raise forms.ValidationError("entered password is wrong")
